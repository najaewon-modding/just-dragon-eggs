...
public record DragonBattleRecord(int dragonNumber, UUID dragonUuid, Optional<UUID> killerUuid, Optional<String> killerName, long killedAt, List<PlayerDamageRecord> playerDamage, List<OtherEntry> otherDamage, double totalHealing, double maxHealth) {
    public static final Codec<DragonBattleRecord> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.DOUBLE.optionalFieldOf("max_health", 200.0).forGetter(DragonBattleRecord::maxHealth)
    ...
    public double healthBalance() {
        return maxHealth + totalHealing - totalDamage();
    }
